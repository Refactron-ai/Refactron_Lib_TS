...
- [x] Narrowing configured outside the command string was out of scope at the time of this ADR. Two thirds of it has since been closed, and the record is corrected here because both this ADR and issue #118 were being read as current.

  **Amendment, 2026-08-21 (issue #137).** An ambient `PYTEST_ADDOPTS` is seen: `ambientSignals` scans it with the runner's own table and gates on the runner that variable feeds. The sentence above stating it "remains out of scope and undetectable" is stale.

  Narrowing in **pytest configuration** is now seen too. `addopts` and `testpaths` are read from `pytest.ini`, `tox.ini`, `setup.cfg` and `pyproject.toml`, and `addopts` goes through the same `scanArgs` as the command line, so a non-narrowing `addopts` stays `full`. This was not a labelling improvement: all four locations produced a **false `SAFE`** on a change the full suite catches, verified with a no-config control that correctly returned `UNSAFE`. Reading configuration was the fix; renaming the `full` field value, the cheaper option #118 proposed, would not have changed the verdict, and the verdict is what every consumer gates on.

  Still out of scope: vitest `include` and jest `testMatch`. They are JavaScript and would need evaluating rather than parsing, and they cannot currently reach a false `SAFE` because coverage is Python-only and a JS or TS change caps at `UNPROVEN` regardless.
