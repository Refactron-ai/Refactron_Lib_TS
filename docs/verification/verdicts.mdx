...
subset of the suite, the verdict floors at `UNPROVEN` no matter how well the
changed code is covered, because a green run of the tests you selected says
nothing about the tests you did not. See [A narrowed test command cannot be
...
| `unknown`  | Not recognised (`make test`, `npm test`, a wrapper script)  | yes         |

`unknown` does not floor the verdict. Flooring every unparsed wrapper would put
`SAFE` out of reach for most projects, so Refactron reports what it knows rather
than refusing outright. This is a real gap: narrowing hidden inside a wrapper
script is not detected. `full` means "this command names no filters", not "this
run was the whole suite" — narrowing configured in `pytest.ini` `addopts`,
`PYTEST_ADDOPTS`, or a vitest `include` is invisible here.
