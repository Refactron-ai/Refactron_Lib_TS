...
`full` still means "this command names no filters", not "this run was the whole
suite". Narrowing configured in `pytest.ini` `addopts` or `testpaths`, a vitest
`include`, or a jest `testMatch` is **not** seen — Refactron reads the command
and the environment, not your config files.
