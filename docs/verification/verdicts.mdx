...
A change consisting _entirely_ of excluded statements does not reach `SAFE`:
there is nothing a test could have proven about it.

<Note>
  This is stricter than it was before 0.5.0. The old rule cleared a whole file as
...
This is a real gap, and two shapes fall into it: `python -m unittest tests.test_foo`
and `python3 tests/runtests.py auth` both narrow and both classify `unknown`.
`full` also means "this command names no filters", not "this run was the whole
suite" — narrowing configured in `pytest.ini` `addopts`, `PYTEST_ADDOPTS`, or a
vitest `include` is invisible here.
