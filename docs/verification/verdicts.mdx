...
`refactron verify-diff --mutate` closes that gap. It perturbs the operators in
your changed statements — a boundary (`<=` → `<`), an arithmetic sign, a boolean
`and`/`or` — reruns your suite against each, and if any mutant **survives** (the
...
(calc.py:2, +->-): no test failed when its behaviour changed. Add a test
that asserts on it.
```
