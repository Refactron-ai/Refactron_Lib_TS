...
- `coverage.uncovered`: one entry per unexercised **statement**, at the statement's first line. Coverage is attributed per statement, so a multi-line statement reports once rather than once per wrapped line.
