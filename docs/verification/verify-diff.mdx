...
| ---------- | --------------------------------------------------------------------------------------------------------- | ---- |
| `SAFE`     | Every gate passed **and** your tests exercise the changed code (at least one changed statement per file). | `0`  |
...
That puts `refactron` (and `refactron-mcp`) on your `PATH`. To skip the install, run `npx refactron@0.3.0 verify-diff ...` instead.
