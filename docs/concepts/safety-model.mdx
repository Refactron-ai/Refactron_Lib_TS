...
**Typical wall-clock:** dominated by the user's own suite size. The test-gate timeout scales with the change's blast radius — 45s for a standard refactor, 120s for a critical one.
