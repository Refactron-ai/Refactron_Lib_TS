...
The verdict is never a model's call. The one place an LLM appears in Refactron is the optional [`document` command](/cli/reference#document) in [migration mode](/transforms/index): after a change has already been verified and written, it generates docstrings, inline comments, and changelog prose on top of the verified diff. The worst case is a clumsy sentence: never a wrong verdict, never broken code.
