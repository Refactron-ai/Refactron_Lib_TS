...
<Update label="Unreleased" description="on main">
  The verification layer lands on main: verify any diff — an AI agent's, a
  codemod's, or your own — for a `SAFE` / `UNSAFE` / `UNPROVEN` verdict, and
  expose the same gate to agents over MCP. Not yet on npm; build from source
  until the next release (`refactron@0.2.4` remains the migration-mode
  transform CLI).
...
- **`verify-diff` command** — verify an arbitrary unified diff end to end and
  ...
  (PR #75)
- **MCP server with the `verify_change` tool** — a stdio server (name
  `refactron`) an AI agent calls before it lands a change; accepts full-file
  `edits` or a `unifiedDiff` and returns the same JSON verdict report. (PR #75)
- **Three-way verdict fusion** — `SAFE` (gates pass and the changed lines are
  ...
  `SAFE`. (PR #75)
- **`preflight` command** — a coverage-aware SQLAlchemy 1.x → 2.0 migration
