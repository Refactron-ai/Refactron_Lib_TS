...
| Layer | Role |
|-------|------|
| **Models** | `CodeIssue`, `BlastRadius`, `VerificationResult`, `AnalysisResult`, pipeline types — see [Models](/api/models) |
| **Orchestrator** | Wires analysis → autofix → verification → backup → atomic write — see [Orchestrator](/api/orchestrator) |
| **Adapters** | `ILanguageAdapter` for language-specific analyze/transform/verify — see [Adapters](/api/adapters) |
...
- [Core concepts](/concepts/blast-radius) — Blast radius and verification behavior
