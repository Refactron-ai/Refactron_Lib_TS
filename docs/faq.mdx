...
[Migration mode](/transforms/index): 20 deterministic AST transforms that both _author_ a mechanical change and _verify_ it through the same gates before an atomic write. They're the demo of the verification engine on real migrations, and they're the `analyze`, `run`, and `document` commands of the CLI. Migration mode and the verification surfaces (`verify-diff`, MCP, `preflight`) ship in one package: `npm install -g refactron@0.3.0`.
