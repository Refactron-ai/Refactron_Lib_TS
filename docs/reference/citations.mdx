...
The 89-scenario study of MITRE Top-25 CWEs that found ~40% of Copilot's top suggestions contained an exploitable vulnerability. The empirical baseline behind Refactron's "no LLM in the refactor path" decision — see [`why-no-llm`](../concepts/why-no-llm).
