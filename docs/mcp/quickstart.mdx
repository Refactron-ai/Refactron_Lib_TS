...
VS Code uses `servers` instead of `mcpServers`, and Zed uses `context_servers`. Per-client file paths and verification steps live on the client pages: [Claude Code](/mcp/claude-code), [Claude Desktop](/mcp/claude-desktop), [Cursor](/mcp/cursor), [Windsurf](/mcp/windsurf), [VS Code](/mcp/vscode), [Cline](/mcp/cline), [Zed](/mcp/zed), [anything else](/mcp/other-clients).
