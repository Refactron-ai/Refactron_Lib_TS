...
<CardGroup cols={2}>
  ...
  <Card title="Claude Desktop" icon="monitor" href="/mcp/claude-desktop">
    Edit `claude_desktop_config.json` and restart the app.
  </Card>
  <Card title="Cursor" icon="mouse-pointer-2" href="/mcp/cursor">
  ...
  <Card title="Cline" icon="bot" href="/mcp/cline">
    Add it from the extension's MCP Servers panel.
  </Card>
  <Card title="Zed" icon="zap" href="/mcp/zed">
    `settings.json` under `context_servers`, not `mcpServers`.
  </Card>
  <Card title="Any other client" icon="plug" href="/mcp/other-clients">
    Anything that can launch a stdio MCP server can run this one.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Instructions for your agent" icon="clipboard-list" href="/mcp/agent-instructions">
    Copy-paste blocks that tell your agent to set this up and to verify before it applies.
  </Card>
