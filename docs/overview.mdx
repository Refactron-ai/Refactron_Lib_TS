...
<CardGroup cols={3}>
  <Card title="SAFE" icon="circle-check">
    Every gate passed **and** your tests exercise the changed lines. Proven safe.
  </Card>
...
The verification surfaces are unreleased on npm today — build from source (see the [quickstart](/quickstart)):
...
<Warning>
  ...
  earn `SAFE` today — it returns `UNPROVEN` ("coverage of the changed code could not be
...
The published `refactron@0.2.4` on npm is this transform CLI (`analyze`, `run`, `document`). See [Migration mode](/transforms) for the catalog.
