...
That puts two binaries on your `PATH`: `refactron` (the CLI, including `verify-diff` and `preflight`) and `refactron-mcp` (the MCP server). To skip the install, run `npx refactron@0.3.0 <command>` instead.
...
<Note>
  ...
  refactron@0.3.0`. When the npm CLI is missing, the shim prints the exact matching install command
...
The agent proposes an edit (as full-file contents or a unified diff), calls `verify_change`, and gets back the same `SAFE` / `UNSAFE` / `UNPROVEN` report, then decides whether to land it. Full setup, for every client, in the [MCP tab](/mcp/index).

## Migration mode

Refactron also ships 20 deterministic AST transforms that both _author_ a change and _verify_ it through the same gates before an atomic write. They live in the package you already installed:
