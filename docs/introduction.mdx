...
```mermaid
flowchart TD
  A([analyze]) -->|scan for legacy patterns| B([run --dry-run])
  B -->|preview the diff — no write| C([run --apply])
  C --> G1{Gate 1<br/>syntax}
  G1 -->|pass| G2{Gate 2<br/>imports}
  G2 -->|pass| G3{Gate 3<br/>tests}
  G1 -->|fail| R[Rejected<br/>tree untouched]
  G2 -->|fail| R
  G3 -->|fail| R
  G3 -->|pass| W[Atomic batch write]
  W --> D([document --apply])
  D -->|docstrings · comments · CHANGELOG · report| E([done])
  W -.->|drift-safe LIFO undo| RB([rollback])
  D -.-> RB
```
