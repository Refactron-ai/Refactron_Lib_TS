...
run --apply .          ← 3 gates → atomic batch write
   ...
   │
   ▼
document --apply       ← LLM writes docstrings, inline comments,
                          CHANGELOG + a modernization report
                          on the already-verified diff
   │
   ▼
rollback               ← drift-safe LIFO undo of the last
                          run / document --apply, if needed
```
