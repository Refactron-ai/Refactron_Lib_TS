...
description: "20 deterministic transforms that both author a mechanical change and verify it through the same gates: the demo of Refactron's verification engine."
---

**Migration mode** is where Refactron both _authors_ a change and _verifies_ it. Each of the 20 transforms rewrites a well-known legacy pattern deterministically, then runs the result through the [same gates](/concepts/safety-model) that back [`verify-diff`](/verification/verify-diff) (syntax, imports, tests) and either lands it atomically or skips itself. They're the demonstration that the verification engine works on real, mechanical migrations.

These are the `analyze`, `run`, and `document` commands of the published CLI. Migration mode and the verification surfaces (`verify-diff`, the MCP server, and `preflight`) ship in one package: `npm install -g refactron@0.3.0`. See the [quickstart](/quickstart#install).
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="python"
    href="/transforms/manual-typecheck-to-hints"
  >
  ...
  <Card
    ...
    icon="python"
    href="/transforms/deprecated-api-requests-to-httpx"
  >
  ...
  <Card
    ...
    icon="js"
    href="/transforms/promise-constructor-to-async"
  >
  ...
  <Card
    ...
    icon="js"
    href="/transforms/string-concat-to-template-literal"
  >
  ...
  <Card
    ...
    icon="js"
    href="/transforms/vue-set-delete-to-assignment"
  >
...
<Note>
  Every transform's "after" output passes [Gate 1 (syntax)](/concepts/safety-model#gate-1-syntax) by
  construction: the refactorer rebuilds the source from a parsed CST/AST. Gates 2 and 3 (imports +
  tests) are what catch regressions the local rewrite couldn't predict.
</Note>
