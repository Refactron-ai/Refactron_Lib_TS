...
This is the CLI published as `refactron@0.2.4` on npm (`analyze`, `run`, `document`). The verification surfaces (`verify-diff`, the MCP server, and `preflight`) are a [from-source build](/quickstart#build-from-source) until the next release.
